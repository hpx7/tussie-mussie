    ...
    render() {
        ...
        return (
            <div>
                ...
                {currentPlayerInfo && (playerState.turn === currentPlayerInfo.name && currentPlayerInfo.drawnCards.length === 0 && !playerState.offer) &&
                ...
                {currentPlayerInfo && (currentPlayerInfo.drawnCards.length > 0) &&
                    <>
                        ...
                        {currentPlayerInfo.drawnCards.map(card => {
                            return <CardComponent key={card.id} val={card} state={playerState} client={client}
                ...
                {currentPlayerInfo && (playerState.offer) &&
                    <>
                        {chooser.name === currentPlayerInfo.name ? <p>Select the which card you want:</p> :
                            <p>{chooser.name} is deciding on an offer:</p>
                        }
                        <CardComponent key={playerState.offer.faceupCard.id} val={playerState.offer.faceupCard} state={playerState} client={client}
                                       clickHandler={chooser.name === currentPlayerInfo.name ? this.selectOffer : (s: CardAction) => {}} />
                        <CardComponent key={playerState.offer.facedownCard.id} val={playerState.offer.facedownCard} state={playerState} client={client}
                                       clickHandler={chooser.name === currentPlayerInfo.name ? this.selectOffer : (s: CardAction) => {}} />
                    </>
                }
